...
def servo_worker():
    ...
    try:
        ...
        servo.angle = 0
        time.sleep(0.5)  # รอให้ Servo เสถียรก่อน
        ...
        while not stop_event.is_set():
            ...
            # Clamp มุมให้อยู่ในช่วงที่กำหนด (ป้องกันค่าผิดปกติ)
            angle = max(SERVO_MIN, min(SERVO_MAX, angle))
            
            # ถ้ามุมเปลี่ยน ให้ค่อยๆ เปลี่ยน
            if angle != last_angle:
                servo.angle = angle
                time.sleep(0.3)  # รอให้ Servo หมุนถึงตำแหน่งและเสถียร
                last_angle = angle
            ...
            # รอ 1.5 วินาที ก่อนเปลี่ยนมุมถัดไป
            for _ in range(15):
                if stop_event.is_set():
                    break
                time.sleep(0.1)
